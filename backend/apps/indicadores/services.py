...
def calcular_tasa_ahorro(ahorro_neto: Decimal, ingreso_neto: Decimal, meta: Decimal = Decimal('20')) -> dict:
    """
...
def calcular_cobertura_emergencia(saldo_fondo: Decimal, gasto_esencial_mensual: Decimal) -> dict:
    """
...
def calcular_presion_gastos_fijos(gastos_fijos: Decimal, ingreso_neto: Decimal) -> Decimal:
    """
...
def _calcular_cuotas_creditos_mes(usuario, mes, anio):
    credito_ids = Credito.objects.filter(
...
def calcular_indicadores_mes(usuario, mes, anio):
    ...
    cobertura = calcular_cobertura_emergencia(
        saldo_fondo, gasto_esencial
    )
...
def generar_diagnostico(indicadores):
    ...
    if ta < 10:
        criticos.append(
            f"Tasa de ahorro crítica ({ta:.0f}%). Revisa gastos variables."
        )
    elif ta < 20:
        criticos.append(
    ...
    if pf > 50:
        criticos.append(
            f"Presión de gastos fijos alta ({pf:.0f}%). Ideal < 50%."
        )
...
def obtener_resumen_deudas(usuario):
    ...
    total_saldo_tarjetas = tarjetas_activas.aggregate(
        s=Sum('saldo_actual')
    )['s'] or Decimal('0')
    ...
    for t in tarjetas_activas:
        ...
        tarjetas_info.append({
            ...
            'limite': t.limite,
            'porcentaje_uso': pct,
            'semaforo': 'verde' if pct < 60 else ('amarillo' if pct < 80 else 'rojo'),
...
def obtener_provisiones_activas(usuario, limite=5):
    ...
    """
    provisiones = Provision.objects.filter(
        usuario=usuario, activa=True
