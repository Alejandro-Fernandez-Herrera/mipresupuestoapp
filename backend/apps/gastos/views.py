...
def listar_gastos(request):
    ...
    gastos_fijos = gastos.filter(tipo="fijo").aggregate(s=Sum("monto"))["s"] or Decimal(
        "0"
    )
    gastos_variables = gastos.filter(tipo="variable").aggregate(s=Sum("monto"))[
        "s"
    ] or Decimal("0")

    categorias = (
        Categoria.objects.filter(visible=True)
        .filter(Q(usuario=request.user) | Q(usuario__isnull=True))
...
def crear_categoria(request):
    ...
    return render(
        request,
        "gastos/registrar_categoria.html",
        {
            "form": form,
...
def eliminar_categoria(request, categoria_id):
    ...
    if categoria.es_sugerida and categoria.usuario is None:
        messages.error(
            request,
            "No puedes eliminar categorías sugeridas del sistema. Puedes ocultarlas.",
        )
        return redirect("gastos:categorias")
    ...
    if Gasto.objects.filter(categoria=categoria).exists():
        messages.error(
            request,
            f'No se puede eliminar "{categoria.nombre}" porque tiene gastos asociados.',
        )
        return redirect("gastos:categorias")
...
def eliminar_rubro(request, rubro_id):
    ...
    if rubro.es_sugerida:
        messages.error(
            request,
            "No puedes eliminar rubros sugeridos del sistema. Puedes ocultarlos.",
        )
        return redirect("gastos:categorias")
    ...
    if Gasto.objects.filter(rubro=rubro).exists():
        messages.error(
            request,
            f'No se puede eliminar "{rubro.nombre}" porque tiene gastos asociados.',
        )
        return redirect("gastos:categorias")
...
def cargar_rubros(request):
    ...
    if categoria_id:
        rubros = Rubro.objects.filter(categoria_id=categoria_id, visible=True).order_by(
            "orden", "nombre"
        )
