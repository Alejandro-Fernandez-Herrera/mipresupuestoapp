...
class TarjetaCredito(models.Model):
    ...
    def disponible(self):
        return (
            Decimal("0")
            if self.saldo_actual >= self.limite
            else self.limite - self.saldo_actual
        )
...
class Credito(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.nombre} — ${self.capital:,.0f} @ {float(self.tasa_ea)*100:.1f}% EA"
        )
...
class CuotaCredito(models.Model):
    ...
    fecha_pago_real = models.DateField(
        null=True, blank=True, verbose_name="Fecha de pago real"
    )
    creado_en = models.DateTimeField(auto_now_add=True)
