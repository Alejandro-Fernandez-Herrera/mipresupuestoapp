...
def listar_creditos(request):
    ...
    return render(request, 'deudas/lista.html', {
...
def _calcular_saldo_actual(credito):
    ...
    if not ultima_cuota:
        cuota = calcular_cuota_mensual(credito.capital, credito.tasa_ea, credito.plazo_meses)
...
def registrar_credito(request):
    ...
    return render(request, 'deudas/registrar.html', {
        'form': form,
...
def detalle_credito(request, credito_id):
    ...
    cuota = calcular_cuota_mensual(credito.capital, credito.tasa_ea, credito.plazo_meses)
    interes_total = calcular_interes_total(credito.capital, credito.tasa_ea, credito.plazo_meses)
    ...
    return render(request, 'deudas/detalle.html', {
        'credito': credito,
        'cuotas': cuotas,
        'cuota_mensual': cuota,
        'interes_total': interes_total,
        ...
        'saldo_actual': saldo_actual,
        'cuotas_pagadas': cuotas_pagadas,
        'progreso': int((cuotas_pagadas / credito.plazo_meses) * 100) if credito.plazo_meses > 0 else 0,
    })
...
def editar_credito(request, credito_id):
    ...
    if credito.cuotas.filter(pagada=True).exists():
        messages.error(request, 'No puedes editar un crédito que ya tiene pagos registrados.')
...
def listar_tarjetas(request):
    tarjetas = TarjetaCredito.objects.filter(usuario=request.user).order_by('-creado_en')
    ...
    return render(request, 'deudas/tarjetas_lista.html', {
        'resultados': resultados,
        'total_saldo': total_saldo,
...
def registrar_tarjeta(request):
    ...
    return render(request, 'deudas/tarjetas_registrar.html', {
        'form': form,
...
def detalle_tarjeta(request, tarjeta_id):
    ...
    return render(request, 'deudas/tarjetas_detalle.html', {
        'tarjeta': tarjeta,
        'compras': compras,
        'disponible': disponible,
        'porcentaje_uso': pct_uso,
...
def registrar_compra_tc(request, tarjeta_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if compra.numero_cuotas > 1:
                compra.monto_cuota = calcular_pago_diferido(compra.monto, compra.numero_cuotas)
    ...
    return render(request, 'deudas/compras_registrar.html', {
        'form': form,
        'tarjeta': tarjeta,
    })
...
def registrar_pago_tc(request, tarjeta_id):
    ...
    return render(request, 'deudas/tarjetas_pago.html', {
        'form': form,
        'tarjeta': tarjeta,
    })
