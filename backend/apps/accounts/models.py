...
class ConfiguracionFiscal(BaseModel):
    ...
    smlv = models.DecimalField(
        max_digits=14, decimal_places=2, verbose_name="SMLV mensual (COP)"
    )
    auxilio_transporte = models.DecimalField(
        max_digits=14, decimal_places=2, verbose_name="Auxilio de transporte (COP)"
    )
    uvt = models.DecimalField(max_digits=14, decimal_places=4, verbose_name="UVT (COP)")
...
class UserProfile(AbstractUser, BaseModel):
    ...
    def save(self, *args, **kwargs):
        if not self.configuracion_fiscal:
            self.configuracion_fiscal = ConfiguracionFiscal.objects.order_by(
                "-anio"
            ).first()
