...
def proyectar_gastos_mes(usuario, mes, anio, escenario=None):
    ...
    if proyecciones.exists():
        ...
        if total > 0:
            ...
            if gastos.exists():
                cats = gastos.values('categoria__nombre', 'categoria__color')\
                    .annotate(total=Sum('monto')).order_by('-total')
                por_categoria = [{'categoria': c['categoria__nombre'],
                                  'color': c['categoria__color'],
                                  'monto': c['total']} for c in cats]
...
def calcular_mes_meta_emergencia(usuario, escenario=None):
    ...
    """
    from apps.provisiones.services import (
        calcular_gasto_esencial_mensual, calcular_meta_niveles,
    )
    ...
    resultado = {}
    for nivel, meta in [('minimo', metas['minimo']),
                         ('recomendado', metas['recomendado']),
                         ('ideal', metas['ideal'])]:
        restante = meta - saldo_actual
        if restante <= Decimal('0'):
            resultado[nivel] = {'alcanza': True, 'mes': hoy.month, 'anio': hoy.year, 'meses_necesarios': 0}
            continue
        if ahorro_mensual <= Decimal('0'):
            resultado[nivel] = {'alcanza': False, 'mes': None, 'anio': None, 'meses_necesarios': None}
            continue
        meses_necesarios = int((restante / ahorro_mensual).quantize(Decimal('1'), rounding=ROUND_HALF_UP))
