...
def gastos_reales(user, db):
    ...
    rubro, _ = Rubro.objects.get_or_create(
        categoria=cat, nombre="Test Rubro",
        defaults={'tipo': 'variable', 'visible': True},
    )
    g1 = Gasto.objects.create(
        usuario=user, categoria=cat, rubro=rubro,
        monto=Decimal("500000"), fecha=date(anio_pasado, mes_pasado, 15),
        tipo='fijo', mes=mes_pasado, anio=anio_pasado,
    )
    g2 = Gasto.objects.create(
        usuario=user, categoria=cat, rubro=rubro,
        monto=Decimal("300000"), fecha=date(anio_pasado, mes_pasado, 20),
        tipo='variable', mes=mes_pasado, anio=anio_pasado,
    )
    return [g1, g2]
...
def test_escenarios_creados(user):
    ...
    for nombre, fi, fg in [('optimista', Decimal('1.1000'), Decimal('0.9500')),
                             ('realista', Decimal('1.0000'), Decimal('1.0000')),
                             ('conservador', Decimal('0.9000'), Decimal('1.1000'))]:
        e = Escenario.objects.create(
            usuario=user, nombre=nombre,
            factor_ingreso=fi, factor_gasto=fg,
        )
...
def test_proyeccion_ingreso_manual(user, escenarios):
    ...
    ProyeccionIngreso.objects.create(
        usuario=user, escenario=escenario,
        mes=12, anio=2026, fuente='nomina',
...
def test_proyeccion_gasto_manual(user, escenarios):
    ...
    ProyeccionGasto.objects.create(
        usuario=user, escenario=escenario,
        mes=12, anio=2026, categoria=cat,
...
@pytest.mark.financiero
@pytest.mark.django_db
def test_escenario_optimista_vs_conservador(user, escenarios, nomina_real, gastos_reales):
    """
