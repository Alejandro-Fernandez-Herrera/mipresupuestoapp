...
def ver_fondo(request):
    ...
    meses_minimo = calcular_meses_para_meta(
        fondo.saldo_actual, aporte_mes, metas["minimo"]
    )
    meses_recomendado = calcular_meses_para_meta(
        fondo.saldo_actual, aporte_mes, metas["recomendado"]
    )
    meses_ideal = calcular_meses_para_meta(
        fondo.saldo_actual, aporte_mes, metas["ideal"]
    )
    ...
    return render(
        request,
        "provisiones/fondo_emergencia.html",
        {
            "fondo": fondo,
...
def listar_provisiones(request):
    ...
    provisiones = Provision.objects.filter(usuario=request.user, activa=True).order_by(
        "fecha_pago"
    )
    ...
    for p in provisiones:
        ...
        maximo = calcular_ahorro_maximo_alcanzable(
            p.ahorro_mensual_disponible, meses_rest
        )
...
def detalle_provision(request, provision_id):
    ...
    rec = calcular_ahorro_mensual_recomendado(
        provision.monto_total, provision.ahorro_acumulado, meses_rest
    )
    maximo = calcular_ahorro_maximo_alcanzable(
        provision.ahorro_mensual_disponible, meses_rest
    )
    progreso = calcular_progreso(provision.ahorro_acumulado, provision.monto_total)
    alcanza, deficit = evaluar_alcanzabilidad(
        provision.ahorro_acumulado, maximo, provision.monto_total
    )
