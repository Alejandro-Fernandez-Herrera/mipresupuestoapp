...
def obtener_aportes_mensuales(usuario, mes, anio):
    ...
    try:
        ...
        total = AporteFondo.objects.filter(fondo=fondo, mes=mes, anio=anio).aggregate(
            s=Sum("monto")
        )["s"] or Decimal("0")
        return total
...
def crear_provisiones_sugeridas(usuario):
    ...
    for data in PROVISIONES_SUGERIDAS:
        try:
            anio = hoy.year
            fecha_pago = date_type(
                anio, data["fecha_pago_mes"], min(data["fecha_pago_dia"], 28)
            )
            if fecha_pago < hoy:
                fecha_pago = date_type(
                    anio + 1, data["fecha_pago_mes"], min(data["fecha_pago_dia"], 28)
                )
...
def generar_diagnostico(indicadores):
    ...
    if indicadores.get("tasa_ahorro", 100) < 10:
        criticos.append(
            f"Tasa de ahorro crítica ({indicadores['tasa_ahorro']:.0f}%). Revisa gastos variables."
        )
    elif indicadores.get("tasa_ahorro", 100) < 20:
        criticos.append(
            f"Tasa de ahorro baja ({indicadores['tasa_ahorro']:.0f}%). Intenta reducir gastos no esenciales."
        )

    if indicadores.get("presion_gastos_fijos", 0) > 50:
        criticos.append(
            f"Presión de gastos fijos alta ({indicadores['presion_gastos_fijos']:.0f}%). Ideal < 50%."
        )

    if indicadores.get("cobertura_emergencia", 99) < 1:
        criticos.append(
            f"Fondo de emergencia crítico. Cobertura de {indicadores['cobertura_emergencia']:.1f} meses. Prioriza este fondo."
        )
